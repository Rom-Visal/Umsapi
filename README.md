...
A comprehensive RESTful web service built with **Spring Boot 3.5**, focusing on **Role-Based Access Control (RBAC)**,
secure authentication, and database optimization.

## 🚀 Key Features

* **Authentication & Authorization:** Secure JWT-based authentication with Access and Refresh tokens.
* **Role-Based Access Control (RBAC):** Managed user roles (Admin, Manager, User) and permissions for secure endpoint
  access.
...
2. **Configure Database (src/main/resources/application.yml):**
   ...
   spring:
     datasource:
       url: jdbc:postgresql://localhost:5432/role_base
